...
//pull out mass slices based on unique filter lines
//this algorithm does not consider intensity thresholds, charge, retention time threshold
...
//pull out mass slices
//this algorithm consider minIntensity, min and max charge, min and max mz
//it report unique list of mass slices
		...
		for(unsigned int j=0; j < samples[i]->scans.size(); j++ ) {
			...
			float rt = scan->rt;
               ...
               // }
                ...
                //if ( slices.size() % 10000 == 0) cerr << "MassSlices count=" << slices.size() << endl;
			} //every scan m/z
			//lastScan = scan;
		} //every scan
	} //every samples
...
//pull out m/z slics based on list of precursors in MS1 space.
//slices are checked for minimum intensity threshold
