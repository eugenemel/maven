...
PathwayWidget::PathwayWidget(MainWindow* parent) 
{
   ...
   //setAcceptDrops(true);
    ...
    if( settings->contains("pathwayId") ) {
        ...
        if ( ! pathwayId.isEmpty() ) setPathway(pathwayId);
    }
...
void PathwayWidget::showConnectedNodes(Node* n, int depth) {
     ...
     for(int i=0; i<edges.size(); i++) { 
         ...
         if ( sn && !sn->isVisible()) { cnodes.insert(sn); }
         if ( dn && !dn->isVisible() ){ cnodes.insert(dn); }
...
void PathwayWidget::setCompound(Compound* c) {
	...
	if (setPathway(pathwayId) == true) { _pathwayId=pathwayId; return; }
	...
	//set title
	QString title = QString(c->name.c_str()) + " " + QString(c->formula.c_str());
	setTitle(title);
...
vector<string> PathwayWidget::getCompoundReactions(Compound* c0 ,int depth) { 
	...
	if ( c0->reactions.size() == 0 ) return rids;
...
void PathwayWidget::showSample(int pos ) {
	...
	if ( samples.size() == 0 ) return;
	if ( pos < 0 ) pos=0;
	if ( pos >= getTimerMaxSteps() ) pos = getTimerMaxSteps()-1;
...
MetaboliteNode* PathwayWidget::addMetabolite(QString id, Compound* c) {
	...
	if (!n || cofactorCheck(id) ) {
       ...
       if (cofactorCheck(id)) n->setCofactor(true);
		...
		if (mw) connect(n,SIGNAL(compoundFocused(Compound*)),mw,SLOT(setUrl(Compound*)));
		if (c)  connect(n,SIGNAL(atomSelected(Compound*,int)),this,SLOT(startAtomTransformationAnimation(Compound*,int)));

		if (c)  connect(n,SIGNAL(compoundFocused(Compound*)),this,SLOT(setCompoundFocus(Compound*)));
		if (c)  connect(n,SIGNAL(expandOnCompound(Compound*)),this,SLOT(expandOnCompound(Compound*)));
		if (c)  connect(n,SIGNAL(nodeMoved(Node*)),this,SLOT(adjustEnzymePositions()));
		if (c)  connect(n,SIGNAL(nodePressed(Node*)),this,SLOT(showTinyPlot(Node*)));
		return n;
	}
...
EnzymeNode* PathwayWidget::addEnzyme(QString id, Reaction* r) {
	...
	if (!n) {
        ...
        if(r) n->setReaction(r);
		n->setId(id);
		n->setNote(id);
		n->setGraphWidget(this);
        nodelist[id]=n;
		connect(n,SIGNAL(enzymeFocused(Reaction*)),SLOT(setTitle(Reaction*)));
      	if ( mw ) connect(n,SIGNAL(enzymeFocused(Reaction*)),mw,SLOT(setUrl(Reaction*)));
        return n;
	}
    return (EnzymeNode*) n;
}
...
EnzymeNode* PathwayWidget::addReaction(Reaction* r) {
		...
		if (r->reactants.size() == 0  || r->products.size() == 0 ) return NULL;
...
bool PathwayWidget::saveLayout() {
	if (_pathwayId.isEmpty()) return false;
	...
	query.prepare("insert into pathways_layout(pathway_id,species_id,x,y) values(?,?,?,?)");
	foreach (Node* n, nodelist ) {
	...
	if ( _pathwayId.contains("CUSTOM_")) {
		...
		query.prepare("insert into pathways(pathway_id,pathway_name,reaction_id,database) values(?,?,?,?)");
		foreach (Node* n, nodelist ) {
			if (n->isEnzyme() ) {
...
bool PathwayWidget::loadLayout(QString pid) {
	...
	query.clear();
	if (loadLayout==false) return false;
...
void PathwayWidget::contextMenuEvent(QContextMenuEvent * event){
	...
	connect(&signalMapper, SIGNAL(mapped(int)), this, SLOT(normalizeNodeSizes(int)));
	...
	while (!xml.atEnd()) {
			...
			if (xml.isStartElement()) {
					...
					if (xml.name() == "species") {
							...
							double charge  = 	xml.attributes().value("charge").toString().toDouble();
							double expectedRt  = 	xml.attributes().value("expectedRt").toString().toDouble();
							...
							if (!c) {
								c = new Compound(id.toStdString(),name.toStdString(),formula.toStdString(),charge);
							}
							c->charge=charge;
							...
							QString name = xml.attributes().value("name").toString();
							if (name.isEmpty() )name=id;
							int reversable  = 	xml.attributes().value("reversable").toString().toInt();
							r = new Reaction("_blank_", id.toStdString(), name.toStdString());
                            ...
                            e = addEnzyme(id,r);
							e->setVisible(true);
							if ( xpos != 0 || ypos != 0 ) { e->setPos(xpos,ypos); _doNewLayout=false; }
							...
							if ( stoch == 0 ) stoch == 1;
							if ( cmap.count(id) && r!= NULL ) r->addReactant(cmap[id],stoch);
                            Node* n = locateNode(id);
                            if ( cofactorCheck(id) ) n = addMetabolite(id,cmap[id]); //each cofactor is a seperate node
                            if (n and e ) {
								Edge* edge = addEdge(n,e,"",e->getReaction());
								if (e->isVisible() ) edge->setVisible(true);
							}

					} else if ( xml.name() == "product" ) {
							int stoch =    xml.attributes().value("stoch").toString().toInt();
							if ( stoch == 0 ) stoch == 1;
							if ( cmap.count(id) && r!= NULL ) r->addProduct(cmap[id],stoch);

                            Node* n = locateNode(id);
                            if ( cofactorCheck(id) ) n = addMetabolite(id,cmap[id]);
                            if (n and e ) {
...
void PathwayWidget::setupPairWiseComparisonMatrix() { 
	...
	for(int r=0; r<nRows; r++ ) {
		for(int c=0; c<nCols;c++ ) {
			...
			float concRatio=0;
			if ( conc0 > 0 && conc1>0 ) concRatio = (conc1/conc0)*2;
...
void PathwayWidget::showAnimationStep(float fraction ) {
	...
	float stepFraction = (fraction*getTimerMaxSteps())-step;
        ...
        if ( step >= ConcMatrix.cols() ) return;
        if ( step >= LabelingMatrix.cols() ) return;
	...
	if ( _encodeVideo ) {
	   ...
	   pixmap.save(QDir::tempPath() + QString("/video/video_%1.png").arg(index), "PNG");
	}
}
...
void PathwayWidget::encodeVideo() 
{
	   ...
	   if (filelist.size() == 0) return;
		...
		QString commandFFmpeg = QString("mencoder mf://%1*.png -mf fps=%3 -lavcopts vcodec=msmpeg4v2:vbitrate=800 -ovc lavc -o \"%2\"").
				arg(QDir::tempPath() + "/video/").
				arg(outputFile).
				arg(fps);
		...
		processFFmpeg->setStandardOutputFile(QDir::tempPath() + "/video/output.txt");
...
void PathwayWidget::getReactionPairs() {
    ...
    if (rpairs.size()) { delete_all(rpairs); rpairs.clear(); }
    ...
    QString sql = tr("select * from reactions_rpairs where reaction_id in(%1)").arg(reactionsIds.join(","));
    //qDebug() << sql;
    QSqlDatabase& db = DB.getLigandDB();
    QSqlQuery query(db);
    query.prepare(sql);
...
void PathwayWidget::startAtomTransformationAnimation(Compound* c, int atomNumber ) {
	stopSimulation();
	clearSelectedAtoms();
	animationQ.clear();
	if(c) { animationQ[c] = atomNumber; }
    _timerId = startTimer(_timerSpeed); // in ms
}
...
void PathwayWidget::showAtomTrasformations(Compound* c, int atomNumber) {
    qDebug() << "showAtomTrasformations() " << c->id.c_str() <<  " " << atomNumber << " RPAIRS=" << rpairs.size();

    if(!c) return;
    QString compoundId(c->id.c_str());
    if (!nodelist.contains(compoundId)) return;
    ...
    for(int i=0; i < rpairs.size(); i++) {
        ...
        if (!nodelist.contains(enzymeId)) continue;
        ...
        if (rpair->species1 == compoundId || rpair->species2 == compoundId) {
            QString compoundTwoId = (rpair->species2 == compoundId) ? rpair->species1 : rpair->species2;
            if(!nodelist.contains(compoundTwoId)) continue;
            if(!nodelist[compoundTwoId]->isMetabolite()) continue;
            ...
            qDebug() << rpair->rpair_id << " " << rpair->species1 << " " << rpair->species2;

            bool swapped=false;
            if (rpair->species2 == compoundId) { swapped=true; }
    ...
    */
    ...
    while (!stream.atEnd())
    {
        ...
        if ( values.size() == 2) {
            ...
            vector<Compound*> compounds = DB.findSpeciesByName(id.toStdString(),db.toStdString());
...
void PathwayWidget::dragMoveEvent(QDragMoveEvent *event){ QGraphicsView::dragMoveEvent(event); event->accept();}

QVector<QPoint> PathwayWidget::getEqualentAtoms(QString rpairId) {
    QSqlDatabase& db = DB.getLigandDB();
    QSqlQuery query(db);
    QString sql = "select atom1n, atom2n, atom1, atom2 from rpairs where rpair_id = ?";
    query.prepare(sql);
