...
void ProjectDB::saveIsotopicEnvelopes(vector<IsotopicEnvelopeGroup>& envelopes) {
    ...
    query0.exec("begin transaction");
    if(!query0.exec("create table IF NOT EXISTS isotopic_envelopes(\
                    isotopicPeakId integer primary key AUTOINCREMENT,\
                    groupId int,\
                    sampleId int,\
                    isotopeName varchar(254),\
                    isotopeMz real,\
                    observedAbundance real,\
                    theoreticalAbundance real \
                    )"))  qDebug() << "Ho... " << query0.lastError();

    QSqlQuery query1(sqlDB);
    query1.prepare("INSERT INTO isotopic_envelopes VALUES("
                   "NULL," //primary key
                   "?,?," // groupId, sampleId,
                   "?,?,?,?);" // isotopeName, isotopeMz, observedAbundance, theoreticalAbundance
                   );

    for (auto envelopeGroup : envelopes) {

        int groupId = envelopeGroup.group->groupId;
        vector<Isotope> isotopes = envelopeGroup.isotopes;

        for (auto it = envelopeGroup.envelopeBySample.begin(); it != envelopeGroup.envelopeBySample.end(); ++it) {

            int sampleId = it->first->sampleId;

            auto envelope = it->second;

            vector<double> intensities = envelope.intensities;

            for (unsigned int i = 0; i < isotopes.size(); i++) {
                Isotope isotope = isotopes[i];
                string isotopeName = isotope.name;
                double isotopeMz = static_cast<double>(isotope.mz);
                double theoreticalAbundance = static_cast<double>(isotope.abundance);

                double observedAbundance = intensities.at(i);

                query1.addBindValue(groupId);
                query1.addBindValue(sampleId);
                query1.addBindValue(QString(isotopeName.c_str()));
                query1.addBindValue(isotopeMz);
                query1.addBindValue(observedAbundance);
                query1.addBindValue(theoreticalAbundance);
