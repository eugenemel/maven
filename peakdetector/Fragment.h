...
class Fragment {
    ...
    void printConsensusNIST(ostream& outstream, double minConsensusFraction, float productAmuToll, Compound* compound=0) {
        if(this->consensus == NULL) return;
		...
		for(unsigned int i=0; i<consensus->mzs.size(); i++ ) { 
			...
			if (fracObserved > minConsensusFraction )  {
                ...
                else outstream << setprecision(6) << consensus->mzs[i] << "\t";
	...
	void addFragment(Fragment* b) { brothers.push_back(b); }
